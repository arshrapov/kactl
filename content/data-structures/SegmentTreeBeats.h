/**
* Author: Rafael Ashrapov
 * Date: 2024-10-18
 * License: CC0
 * Source: folklore
 * Description: Svatoslav should know all those shit
 * Time: O(\log N)
 * Status: notstress-tested
 */
class ExtendedJiDriverSegmentTree {
	...
	void build(const int v, const int l, const int r, const vt<int64_t>& inputArray) {
		tree[v].pushSum = 0;
		if (l + 1 == r) {
	...
	public:
	ExtendedJiDriverSegmentTree(const vt<int64_t>& inputArray) {
		n = inputArray.size();
		tree.resize(4 * n);
